...
describe('public messages copy', () => {
  ...
  it('keeps hardcoded homepage and CLI page counts aligned with current IDE support', () => {
    ...
    expect(homeSource).toContain('19+');
    expect(homeSource).not.toContain('15+');
    expect(cliSource).toContain('19+ IDEs');
    expect(cliSource).not.toContain('15+ IDEs');
    expect(integrationsSource).toContain('19+ IDE');
  ...
  it('keeps CLI FAQ and integrations entry copy focused on skills-first onboarding', () => {
    const cliSource = readPageSource('../pages/[locale]/cli/index.astro');
    const integrationsSource = readPageSource('../pages/[locale]/integrations/index.astro');
    ...
    expect(cliSource).toContain(
      'Use the Killer-Skills CLI to install, manage, and sync AI agent skills in Cursor, Claude Code, VS Code, and Windsurf for process automation and template-based workflows, with no global install.',
    );
    expect(cliSource).not.toContain('install, configure, and sync AI agent skills');
    expect(cliSource).toContain('The CLI detects your IDE automatically and writes the correct IDE-native skill file.');
    expect(cliSource).toContain('CLI 会自动识别你的 IDE，并写入正确的 IDE 原生技能文件。');
    expect(cliSource).not.toContain('CLI 会自动识别你的 IDE，并生成对应的技能文件。');
    expect(cliSource).toContain('detected IDE target and prepared IDE-native skill file');
    expect(cliSource).not.toContain('detected ide target and prepared native skill file');

    expect(integrationsSource).toContain('this page is the skills-first compatibility overview');
    expect(integrationsSource).toContain('writes the correct IDE-native skill format');
    expect(integrationsSource).toContain('这里就是技能优先的兼容性总览页');
    expect(integrationsSource).toContain('写入正确的 IDE 原生技能格式');
    expect(integrationsSource).toContain("'IDE-Native Skill Setup'");
    expect(integrationsSource).toContain("'IDE 原生技能配置'");

    expect(en.Integrations.heroDesc).toBe(
      'Use Killer-Skills across major AI coding tools and IDEs with the same reusable skills.',
    );
    expect(en.Integrations.missingTool).toBe("Don't See Your IDE?");
    expect(en.Integrations.learnProtocol).toBe('See How Skills Integrate');
    expect(en.Integrations.missingToolDesc).toBe(
      'If your IDE can run skill instructions, Killer-Skills can usually work with it.',
    );

    expect(zh.Integrations.heroDesc).toBe('在主流 AI 编码工具和 IDE 中复用同一套 Killer-Skills 技能。');
    expect(zh.Integrations.missingTool).toBe('没看到你的 IDE？');
    expect(zh.Integrations.learnProtocol).toBe('查看技能接入方式');
    expect(zh.Integrations.missingToolDesc).toBe('只要 IDE 能运行技能说明，Killer-Skills 通常就能用。');
  });
