    ...
    <div className="bg-white dark:bg-[#1e1e1e] rounded-xl overflow-hidden border border-gray-200 dark:border-white/5 shadow-lg dark:shadow-none flex flex-col min-h-[200px] h-auto transition-colors duration-300">
      ...
      <div className="h-10 bg-gray-100 dark:bg-[#252526] flex items-center justify-between px-4 border-b border-gray-200 dark:border-black/20 shrink-0 transition-colors duration-300">
        <div className="flex items-center gap-4">
          ...
          <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest hidden md:inline-block select-none">
            {labels.explorer}
          </span>
        </div>
      </div>
...
`}
              >
                <div className="flex items-center gap-2.5 overflow-hidden">
                  {file.type === 'folder' ? (
                    ...
                    <FileText
                      size={15}
                      className={
                        selected === file.name ? 'text-cyan-500 dark:text-cyan-400' : 'text-gray-400 dark:text-gray-500'
                      }
                    />
                  )}
                ...
                {file.size && (
                  <span
                    className={`text-[10px] ${selected === file.name ? 'text-cyan-500/70 dark:text-cyan-400/70' : 'text-gray-400/50 dark:text-gray-600 group-hover:text-gray-500 dark:group-hover:text-gray-500'} font-mono`}
                  >
                    {file.size}
                  </span>
                )}
