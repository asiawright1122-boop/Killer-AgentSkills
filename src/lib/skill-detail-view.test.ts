...
describe('buildMarketplaceDetailTrust', () => {
  it('builds install decision trust evidence for detail pages', () => {
    ...
    expect(trust.rows.map((row) => row.label)).toContain('Blocking risk');
    ...
    expect(trust.rows.find((row) => row.label === 'Install path')?.value).toBe('Checked: owner/repo');
    expect(trust.riskLabels).toEqual(['Network access']);
    expect(trust.whyListed).toContain('blocking risks were checked');
  });
});
