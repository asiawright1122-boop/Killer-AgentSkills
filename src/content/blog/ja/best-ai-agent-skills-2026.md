...
description: "Discover the best AI models in 2026, tested across Claude, Cursor, and Windsurf. Get the essential list of what they do well, sorted by performance."
pubDate: 2026-02-23
...
**AIエージェントスキル**は、特殊なプラグアンドプレイのインストラクションモジュールで、コードアシスタント（例：Claude Code、Cursor、Windsurf）に複雑なワークフローを自律的に実行するためのコンテキストと機能を提供します。Killer-Skillsレジストリからの最近のデータによると、ターゲットを絞ったエージェントスキルを使用する開発者は、繰り返しのフォーマット、テスト、ドキュメント作成タスクで平均12.5時間/週を節約できると報告しています。
...
> - **ユニバーサル互換性**: `npx killer-skills add owner/repo`を使用して、19以上のIDEにグローバルにスキルをインストールします。
## AIエージェントスキルとは何か？
...
私たちは、2,500を超えるエージェントスキルのディレクトリを維持しており、毎日数十個を使っています。いくつかは優れています。多くは中途半端です。幾つかは私たちの仕事のやり方を変えました。
...
完全なコレクションは、[killer-skills.com/ja/skills](/ja/skills) で参照できます。
...
これらのスキルは、Cursor、Windsurf、VS Code（CopilotまたはCline経由）、Trae、Claude Code CLIを含む19以上の主要なAIコーディング環境と互換性があります。
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "これらのスキルは、Cursor、Windsurf、VS Code（CopilotまたはCline経由）、Trae、Claude Code CLIを含む19以上の主要なAIコーディング環境と互換性があります。"
      }
    },
    {
      "@type": "Question",
