...
MCP 서버를 잘 만든다는 것은 긴 체크리스트를 복사하는 일이 아니라, 어떤 기능을 도구로 노출할지, 에이전트가 이해하기 쉬운 설명과 스키마를 어떻게 만들지, 그리고 운영 중 어떤 제약을 둘지를 먼저 정하는 일에 가깝습니다.
...
전역 CLI 설치는 필요하지 않습니다. `npx`로 mcp-builder 스킬을 바로 추가하세요:

```bash
npx killer-skills add anthropics/skills/mcp-builder
```

스킬은 `.claude/skills/` 디렉터리에 추가되고 Claude가 MCP 서버 개발 작업을 감지하면 자동으로 활성화됩니다.
...
3. 적절한 OAuth 인증이 포함된 완전한 서버를 구축합니다
...
에이전트는 집중되고 관련성 높은 데이터에서 가장 좋은 성능을 냅니다:
...
시작하는 가장 빠른 방법은 Killer-Skills 스킬 디렉터리를 이용하는 것입니다:
...
---
*더 많은 스킬을 찾고 싶다면 [Killer-Skills 스킬 디렉터리](https://killer-skills.com/ko/skills)를 둘러보고 AI 코딩 워크플로우를 위한 수백 개의 검증된 에이전트 스킬을 발견하세요.*
