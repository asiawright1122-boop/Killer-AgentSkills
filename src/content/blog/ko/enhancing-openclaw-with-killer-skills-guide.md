...
Node.js가 시스템에 설치되어 있는지 확인하세요. 이 워크플로에서는 전역 CLI 설치가 필요 없으며, 프로젝트 안에서 `npx`로 Killer-Skills를 바로 실행하면 됩니다.
...
`npx killer-skills sync --ide openclaw` 명령은 OpenClaw에서 이해할 수 있는 XML 프롬프트 블록 세트를 자동으로 생성하고 `AGENTS.md`에 삽입합니다.
## 시나리오 기반 기술 팩
...
[스킬 디렉터리](https://killer-skills.com/ko/skills)로 이동하여 다음 "슈퍼파워"를 선택하세요!
