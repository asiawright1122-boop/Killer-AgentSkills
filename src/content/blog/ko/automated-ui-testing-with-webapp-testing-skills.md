...
- 로컬 서버를 자동으로 시작하고 중지합니다 (예: `npm run dev`).
- 여러 서버를 동시에 관리합니다 (예: Frontend + Backend).
- 네트워크가 유휴 상태이고 애플리케이션이 준비될 때까지 테스트가 실행되지 않도록 보장합니다.
...
### 복잡한 상호작용 흐름
"회원가입 -> 결제 -> 대시보드 보기"와 같은 멀티스텝 사용자 여정을 자동화하여 애플리케이션의 핵심 비즈니스 논리가 깨지지 않은지 확인합니다.
