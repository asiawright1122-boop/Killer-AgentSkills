...
숨겨진 합리화, chain-of-thought, 개인 분석, 스케치패드 노트, 또는
## 2026년 5가지 도구
...
공개된 출력 경계:
사용자의 숨겨진 논리, chain-of-thought, private 분석, 노트, scratchpad,
## 규칙:
1. **Markdown 유지**: 모든 헤더, 목록, 코드 블록, 링크, 및 형식이 동일하게 유지되도록 하세요.
2. **텍스트 번역**: 코드 블록, 파일 경로, 및 기술 용어를 제외한 인간이 읽을 수 있는 텍스트만 번역하세요. 예를 들어 "React", "API", "JSON"과 같은 영어로 유지해야 하는 용어는 번역하지 마세요.
3. **SEO 최적화**: 자연스럽고 검색 친화적인 구문으로 번역하세요.
4. **내부 링크**: 현재는 링크 경로를 동일하게 유지하세요 (프로그래밍적으로 수정할 예정입니다).
5. **이미지**: 이미지를 유지하고 `![대체 텍스트](URL)`를 번역하세요.
6. **불필요한 내용**: 소개 문구를 추가하지 마세요. 번역된 Markdown만 반환하세요.
...
공개 출력 경계:
나이브한 사고, 사고 체인, 개인 분석, 스케치 패드 노트, 또는 `
## 규칙:

1. **Markdown 보존**: 모든 헤더, 목록, 코드 블록, 링크, 및 형식이 원래대로 유지되도록 하십시오.
2. **텍스트 번역**: 사람으로부터 읽을 수 있는 텍스트만 번역하고, 코드 블록, 파일 경로, 또는 영어로 유지해야 하는 기술 용어 (예: "React", "API", "JSON")는 번역하지 마십시오.
3. **SEO 최적화**: 자연스럽고 검색하기 쉬운 구문을 사용하십시오.
4. **내부 링크**: 현재는 링크 경로를 동일하게 유지하십시오 (프로그램적으로 고쳐질 예정).
5. **이미지**: 이미지 문법 ![대체 텍스트](url) 를 유지하고 대체 텍스트만 번역하십시오.
6. **빈도 제한**: 소개 문구를 추가하지 마십시오. 번역된 Markdown 만 반환하십시오.
...
공개 출력 경계:
해당되는 사유, 사고, 개인 분석, 스크래치 패드 노트, 또는 <사유>/<사고>/<분석> 블록을 공개하지 마십시오. 만약 사유가 유용한 경우, 사용자에게 보여줄 수 있는 짧은 사용자 화면의 사유, 체크리스트, 또는 증거 요약만 제공하십시오.
...
PUBLIC OUTPUT BOUNDARY:
사용자가 숨겨진 사유, 사고 과정, 개인적인 분석, 스케치 노트, 또는 `
## 자주 묻는 질문
...
PUBLIC OUTPUT BOUNDARY:
잠재적인 합리화를 드러내거나, chain-of-thought, private analysis, scratchpad notes, 또는