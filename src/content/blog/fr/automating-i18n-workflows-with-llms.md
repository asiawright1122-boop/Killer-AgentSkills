...
Historiquement, l'exécution de scripts de vérification hors ligne et de routines de synchronisation a naturellement conduit à une logique de code à courte vue. Par exemple, notre script `clean-broken-skills.js` maintenait activement une matrice de locale interne `const locales = ['zh', 'ja', 'ko'];`, ce qui a rendu aveugles les métriques du système pour d'autres démographies comme l'arabe, l'hindi et le portugais. Lorsque la plateforme a été mise à l'échelle, cela a créé un vide massif dans la couverture de secours SSR. En adoptant un modèle de [Developer Experience](/en/skills/owner/repo/) ouvert, nous avons reconnu que les scripts nécessitaient un pipeline central `SUPPORTED_LOCALES`.
...
Pour découvrir une interface localisée de manière transparente et performante sur 11 localisations entièrement automatisées, visitez le portail principal [Killer-Skills](/en/). En adoptant une localisation automatisée continue pilotée par les agents, nous nous assurons que notre flux de travail et nos plugins d'IA sont accessibles de manière démocratique dans le monde entier.