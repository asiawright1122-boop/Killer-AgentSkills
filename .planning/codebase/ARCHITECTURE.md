...
- Contains: `src/lib/skills.ts`, `src/lib/kv.ts`, `src/lib/search.ts`, public projection/sanitizer helpers, taxonomy/intent utilities.
...
- Purpose: Ensure data from D1, KV, local cache, GitHub, and AI providers is filtered and sanitized before it reaches public pages, API payloads, browser UI, cache artifacts, or generated seed files.
- Examples: `getLightweightSkills*` wrappers in `src/lib/skills.ts`, `src/lib/public-ai-output.ts`, `src/lib/public-skill-api.ts`, and `src/lib/public-skill-copy.ts`.
...
- Public AI output boundaries are enforced at API edges. Runtime previews must return final answers, concise rationale, and evidence only; hidden reasoning, chain-of-thought, scratchpad notes, and model-internal deliberation are not valid public payloads.
- Shared sanitizers in `src/lib/public-ai-output.ts` and `src/lib/public-skill-api.ts` protect generated copy, public skill projections, nested JSON records, API error payloads, GitHub file metadata, cached AI outputs, D1 seed generation, and public cache verification paths.
- Client islands are also public output boundaries. Caught exception messages may be logged for operators, but `ErrorBoundary`, WebContainer terminals, modals, and other browser UI must render fixed public copy or API-returned public validation messages rather than raw `error.message` text.
- Public-surface CI guards scan source content, skill cache projections, D1 seed SQL, and built `dist/client` artifacts so hidden reasoning markers fail before deployment.
...
_Update when major data flow or layering changes_
