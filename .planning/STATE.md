...
**Current focus:** The project is in `v1.8` and should refresh proof inputs and comparable recovery evidence before any authority expansion or automation decision changes.

## Current Position

**Current Milestone:** `v1.8` Fresh Recovery Inputs and Comparable Proof Refresh
**Current Phase:** `61` coverage-drilldown-input-refresh-and-freshness-contract
**Current Plan:** none
**Status:** Milestone `v1.8` is initialized. Requirements and roadmap are defined, and Phase `61` is ready for discussion and planning.
**Last Activity:** 2026-04-23
**Last Activity Description:** Started `v1.8`, defined active requirements, and created the three-phase roadmap covering fresh inputs, comparable proof refresh, and readiness reassessment.
**Progress:** 0%
...
- The freshest local Coverage Drilldown raw export is still dated `2026-04-03`, so cluster-level recovery proof remains partially blocked by stale inputs.
...
- Discovery expansion remains closed at `0 promote / 16 hold / 1 stop`, and automation remains locked.
...
- `v1.8` is the active path to resolve the next blocking lane: stale recovery inputs and incomplete comparable proof.
...
| v1.8 posture | Refresh proof inputs before revisiting promotion or rollout posture | Recovery claims still depend on stale inputs and one seeded comparable window |
...
- No active blocker remains inside the `v1.7` public trust-surface scope; the active blocker is stale proof input freshness and missing comparable evidence for the recovery lane.
...
**Stopped At:** `v1.8` initialization is complete. The next GSD action is `$gsd-discuss-phase 61` or `$gsd-plan-phase 61`.
