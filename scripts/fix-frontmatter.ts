...
async function fixFrontmatter() {
  ...
  const locales = fs
    ...
    .filter((d) => fs.statSync(path.join(BLOG_DIR, d)).isDirectory() && d !== 'en');
...
- Keep the wording natural for ${locale}
...
Original Description: "${enDesc}" (${enDesc.length} characters)`;
          ...
          try {
            ...
            const localizedCurrentDesc = locDescMatch?.[1] ?? '';

            newDesc = sanitizeMetaDescription(newDesc, locale);
            newDesc = trimDescriptionToMax(newDesc, maxLen);

            if (newDesc.length < minLen) {
              const localizedFallback = trimDescriptionToMax(
                sanitizeMetaDescription(localizedCurrentDesc, locale),
                maxLen,
              );
              if (localizedFallback.length >= newDesc.length) {
                console.log(
                  `     ⚠️ Description short (${newDesc.length}); using existing localized description (${localizedFallback.length})`,
                );
                newDesc = localizedFallback;
              } else {
                console.log(
                  `     ⚠️ Description short (${newDesc.length} chars, expected ${minLen}-${maxLen}); keeping generated localized text`,
                );
              }
            }
