...
- Keep the language clear and natural for English readers
...
Original Description: "${desc}" (currently ${desc.length} characters)`;
  ...
  const { min: minLen, max: maxLen } = getDescriptionLengthRange('en');
  newDesc = sanitizeMetaDescription(newDesc, 'en');
  newDesc = trimDescriptionToMax(newDesc, maxLen);

  if (newDesc.length < minLen) {
    const extension = ' Learn practical setup steps and real examples you can apply immediately.';
    const expanded = trimDescriptionToMax(`${newDesc} ${extension}`, maxLen);

    if (expanded.length >= minLen) {
      newDesc = expanded;
    } else {
      console.log(`     ⚠️ Description remains short (${newDesc.length} chars, expected ${minLen}-${maxLen})`);
    }
  }
