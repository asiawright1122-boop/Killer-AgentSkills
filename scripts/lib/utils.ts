...
export function robustParseJSON(str: string): any {
  ...
  try {
    ...
    // Try 2: Repair truncated JSON while staying inside JSON.parse.
    try {
      const repaired = repairTruncatedJSON(str);
      return JSON.parse(repaired);
    } catch {
      // Try 3: Quote simple unquoted keys if missing (last resort).
      try {
        const quoted = str.replace(/([{,]\s*)(\w+)(\s*:)/g, '$1"$2"$3');
        return JSON.parse(repairTruncatedJSON(quoted));
      } catch {
        return null;
      }
    }
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (
        ...
        (error.message?.startsWith('Server error: 4') && !error.message?.includes('429'))
      ) {
        throw error;
      }
